...
export async function resolveOptions(cli: Partial<InstallOptions> & { yes: boolean }, ask: Prompter): Promise<InstallOptions> {
  ...
  const errors = validateSelection({ providers, defaultProvider, vectorStore });
  if (errors.length) throw new Error(errors.join(" "));

  return { projectName, providers, defaultProvider, vectorStore, git: cli.git ?? post.git, install: cli.install ?? post.install, packageManager, yes: cli.yes };
}
