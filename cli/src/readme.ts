...
export function generateReadme(o: InstallOptions): string {
  ...
  lines.push("  hour per IP). `0` disables a limit. These guard your provider budget — but read");
  lines.push("  **Rate limits** below, they are not as strong as they look.");
  ...
  lines.push("unlimited, it starts enforcing 20 chat requests/minute, 200/day per user, and 5");
  lines.push("registrations/hour per IP the moment you run `db:migrate` — a user who was");
  lines.push("sending 250 messages a day will start getting 429s with no warning. Set any of");
  lines.push("the three to `0` in **Settings** to disable it.", "");
  lines.push("The registration limit only binds behind a reverse proxy that OVERWRITES");
  lines.push("`x-forwarded-for` (Vercel, Fly, or nginx with `proxy_set_header X-Forwarded-For");
  lines.push("$remote_addr;`). The commonly copy-pasted `proxy_set_header X-Forwarded-For");
  lines.push("$proxy_add_x_forwarded_for;` recipe APPENDS instead, so the client's own forged");
  lines.push("value stays first in the chain and a fresh header per request dodges the limit");
  lines.push("entirely. With no reverse proxy at all (`npm run dev`, a bare `next start`),");
  lines.push("there is no `x-forwarded-for` and the check is skipped — deliberately, to avoid");
  lines.push("locking out every anonymous caller behind one shared bucket (see the `clientIp`");
  lines.push("comment in `src/app/api/register/handler.ts`). In either broken case the admin");
  lines.push("panel still shows whatever number you configured; it is simply not enforced. If");
  lines.push("mass signups matter to you, use invitations or email verification — this limit");
  lines.push("only raises the cost of scripted registration, it does not remove it.", "");
  lines.push("The per-user chat cap bounds one account, not your total spend: registering is");
  lines.push("effectively free, and the two limits compose (5 accounts/hour/IP × 200");
  lines.push("msgs/day/account is up to 24,000 chat requests/day from one IP, even behind a");
  lines.push("correctly overwriting proxy). Bounding total spend means restricting who can");
