...
describe("listWorkspaceUsersResponse", () => {
  it("returns users with their granted flag", async () => {
    const res = await listWorkspaceUsersResponse("w1", { getAdmin: admin as never, listWorkspaceUsersFn: (async () => [{ id: USER_ID, email: "a@x", granted: true }]) as never });
  ...
  it("404s on an unknown workspace", async () => {
    ...
    const res = await listWorkspaceUsersResponse("nope", { getAdmin: admin as never, listWorkspaceUsersFn: listWorkspaceUsersFn as never });
