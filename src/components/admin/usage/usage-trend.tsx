...
export function UsageTrend({ points }: { points: UsageTrendPoint[] }) {
  if (points.length === 0) {
    return <p className="text-sm text-zinc-500">No recorded usage in the last {USAGE_WINDOW_DAYS} days.</p>;
  }
      ...
      {points.map((p) => {
        ...
        return (
          <li key={p.day} className="flex items-center gap-2 text-xs">
            ...
            <span className="w-20 shrink-0 text-right text-zinc-500">{p.totalTokens.toLocaleString("en-US")}</span>
          </li>
        );
      })}
    </ul>
