...
// Verify credentials against the DB. Framework-agnostic (no next-auth import) so
// both the full-app NextAuth Credentials provider (src/auth.ts) and the api-only
// login endpoint (src/api/auth/login/handler.ts) can call it directly.
